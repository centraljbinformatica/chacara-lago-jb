...
- **Revisão geral (jul/2026)** — João aparece como "João Belém (JB)" na apresentação/reserva (pra ligar o nome ao "JB") e como "JB" no rodapé/contato. Atrações voltaram a citar "karaokê em breve" e "novidades chegando". Link "Contato" adicionado ao menu do topo. Stats "25+" → "30+" itens. `theme-color` unificado (#1d3f70) e meta description do cardápio melhorada. Dados do Google (JSON-LD): + horário (sex/sáb/dom a partir das 10h), url, priceRange, image e link do cardápio (`hasMenu`). Falta o **horário de fechamento** (a confirmar com a dona).
- **Ajustes de copy** — "domingo" virou "fim de semana" nos textos (hero, sobre, faixa de foto, mensagem do WhatsApp), já que agora abre sex/sáb/dom. Card "Quando abre" atualizado pra "Sextas, sábados e domingos (e alguns feriados e dias de festival), a partir das 10h". Seção dos proprietários: eyebrow virou "Fale através dos canais oficiais" e o texto ficou mais direto. Em "Onde estamos": card virou "WhatsApp Proprietários" e o João aparece como "JB".

- **Seção "Fale com os proprietários"** — seção própria (`#proprietarios`) num ponto estratégico mais acima (logo depois das atrações), deixando claro de forma sutil quem são os donos: **João Belém** ((92) 99215-2111) e **Hailer Belém** ((92) 99396-7080). Texto curto ("Precisa de reserva ou falar sobre algo do local?...") + botões de WhatsApp (João primeiro, Hailer depois) e um botão pro Instagram oficial. Motivo: gente se passando por dono; aqui fica claro que os canais oficiais são só esses. Os nomes também aparecem no texto da seção de reserva, e os botões continuam na seção "Onde estamos" (agora com João antes da Hailer). O box `.owner-note` antigo (aviso 📌) foi removido por ser pesado demais.
